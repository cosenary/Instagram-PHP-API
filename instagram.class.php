 ...
 *
 * Modified and Fixed for SocialModule
 * @link https://github.com/kaerer/Yii-SocialShell GitHub address of the module that uses this
 *
...
class Instagram {
    ...
    public function searchUser($name, $limit = 0) {
        $params = array_merge(array('q' => $name, 'count' => $limit), $this->_callparams);
    ...
    public function getUserFeed($limit = 0) {
        $params = array_merge(array('count' => $limit), $this->_callparams);
    ...
    public function searchMedia($lat, $lng, $distance = 1000) {
        $params = array_merge(array('lat' => $lat, 'lng' => $lng, 'distance' => $distance), $this->_callparams);
        return $this->_makeCall('media/search', false, $params);
    }
    ...
    public function searchTags($name) {
        $params = array_merge(array('q' => $name), $this->_callparams);
        return $this->_makeCall('tags/search', false, $params);
    }
    ...
    public function addParam($key, $value) {
        $this->_callparams[$key] = $value;
    }

    public function cleanParam() {
        $this->_callparams = array();
    }
